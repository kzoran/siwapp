#!/bin/bash
#===============================================================================
# Siwapp Installation Script (Elixir version) - App Node 3
...
if [ ! -d "$APP_DIR/.git" ]; then
  ...
  sudo -u "$APP_USER" -H bash -c "
    ...
    export HTTP_PROXY='$PROXY_URL'
    export HTTPS_PROXY='$PROXY_URL'
    git clone 'https://github.com/siwapp/siwapp.git' '$APP_DIR' && cd '$APP_DIR' && git checkout 1aed7462f78368c630be3817ddde8f23118c113d
  "
else
...
if [ -f /etc/default/siwapp ] && grep -q "SECRET_KEY_BASE=" /etc/default/siwapp; then
  ...
  sudo -u "$APP_USER" -H bash -c "
    ...
    export DATABASE_URL=ecto://$APP_USER:$DB_PASSWORD@$DB_HOST/siwapp_prod
    cd $APP_DIR
    MIX_ENV=$MIX_ENV mix phx.gen.secret
  " > /tmp/siwapp_secret
...
# ===============================
# Build Release (no DB setup - app1 handles that)
# ===============================
...
echo "✅ Siwapp App3 installation completed successfully!"
